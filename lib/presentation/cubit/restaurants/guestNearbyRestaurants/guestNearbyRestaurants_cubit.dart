import 'package:local_basket/core/data/repository_cache.dart';
import 'package:local_basket/domain/usecase/restaurants/guestNearbyRestaurants/guestNearbyRestaurants_usecase.dart';
import 'package:flutter_bloc/flutter_bloc.dart';
import 'guestNearbyRestaurants_state.dart';

class GuestNearByRestaurantsCubit extends Cubit<GuestNearByRestaurantsState> {
  final GuestNearByRestaurantsUseCase useCase;
  final RepositoryCache _repositoryCache = RepositoryCache();

  GuestNearByRestaurantsCubit(this.useCase)
      : super(GuestNearByRestaurantsInitial());

  Future<void> fetchGuestNearbyRestaurants(Map<String, dynamic> params, {bool forceRefresh = false}) async {
    // Generate cache key based on parameters
    final cacheKey = _generateCacheKey(params);
    print('🚀 fetchGuestNearbyRestaurants called with cacheKey: $cacheKey, forceRefresh: $forceRefresh');
    
    // Try to get cached data first (unless force refresh)
    if (!forceRefresh) {
      print('🔍 Checking repository cache for guest restaurants data...');
      final cachedData = await _repositoryCache.getData(
        cacheKey,
        (json) => json, // Guest restaurants might use a different model structure
      );
      
      if (cachedData != null) {
        print('📦 Using repository cached guest restaurants data');
        emit(GuestNearByRestaurantsSuccess(cachedData));
        return;
      } else {
        print('❌ No valid cached data found, fetching from API');
      }
    } else {
      // Clear cache for this specific request
      await _repositoryCache.refresh(cacheKey);
      print('🔄 Force refreshing guest restaurants data');
    }

    // Fetch from API if no cached data or force refresh
    print('🌐 Fetching guest restaurants from API...');
    emit(GuestNearByRestaurantsLoading());
    try {
      final result = await useCase(params);
      print('✅ API returned guest restaurants data');
      
      // Cache the result for future use (30 minutes)
      print('💾 Caching guest restaurants data in repository for 30 minutes...');
      await _repositoryCache.setData(cacheKey, result, ttl: const Duration(minutes: 30));
      
      emit(GuestNearByRestaurantsSuccess(result));
    } catch (e) {
      print('❌ Error fetching guest restaurants: $e');
      emit(GuestNearByRestaurantsFailure(e.toString()));
    }
  }

  /// Generate unique cache key based on request parameters
  String _generateCacheKey(Map<String, dynamic> params) {
    final keyParts = <String>[];
    params.forEach((key, value) {
      if (value != null) {
        keyParts.add('$key=$value');
      }
    });
    return 'guest_nearby_restaurants_${keyParts.join('_')}';
  }

  /// Force refresh guest restaurants data
  Future<void> refreshGuestNearbyRestaurants(Map<String, dynamic> params) async {
    await fetchGuestNearbyRestaurants(params, forceRefresh: true);
  }
}
